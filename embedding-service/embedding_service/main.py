...
async def run() -> None:  # pragma: no cover - runtime (запуск серверов)
    ...
    app = build_consumer(
        ...
        lambda: ready["value"],
        reranker_ready=(
            (lambda: reranker_ready["value"]) if deps.reranker_enabled else None
        ),
    )
...
async def _prepare_reranker(  # pragma: no cover - runtime (прогрев/health)
    deps: Deps,
    health: object,
    reranker_ready: dict[str, bool],
) -> RerankerServicer | None:
    """Прогревает reranker изолированно; сбой не влияет на embeddings."""
    if not deps.reranker_enabled:
        return None
    ...
    except Exception as exc:  # любой сбой загрузки reranker — не фатален
