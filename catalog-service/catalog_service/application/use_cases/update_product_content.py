"""Use case ``UpdateProductContent`` (C2)."""
...
class UpdateProductContent:
    """Частично меняет контент товара и публикует событие."""
    ...
    async def execute(self, cmd: UpdateProductContentCommand) -> CommandResult:
        """Выполняет обновление контента (диффом).
        ...
        async with self._uow as uow:
            ...
            if cmd.category_name is not None:
                ...
                category_ref = CategoryRef(category.id, category.name)
            brand_ref = None
            if cmd.brand_name is not None:
                brand = await uow.brands.get_or_create(cmd.brand_name)
                brand_ref = BrandRef(brand.id, brand.name)
