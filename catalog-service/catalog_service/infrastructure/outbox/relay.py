...
публикации). Успех -> ``published_at``; ошибка -> инкремент ``attempts``
и экспоненциальный backoff; исчерпание попыток -> ``failed_at`` (карантин).
"""
...
from catalog_service.infrastructure.messaging.broker import EventPublisher

_BACKOFF_CAP_S = 300


class OutboxPublisher:
    """Публикует накопленные события из outbox в брокер."""
        ...
        async with self._sessionmaker() as session, session.begin():
            stmt = (
                select(
                    ...
                    OutboxORM.aggregate_version,
                    OutboxORM.event_version,
                    OutboxORM.payload,
                    OutboxORM.attempts,
                )
                .where(
