...
- **Integration-тесты (`testcontainers`)**: реальный Qdrant (round-trip,
  ...
  реальный RabbitMQ (доставка + retry/DLQ-топология) — `uv run pytest -m
  integration` (нужен Docker).
- **Ops**: `/health` + `/metrics` (Prometheus), проверено e2e в
  docker-compose (публикация события → точка в Qdrant → инкремент метрики).

**Не реализовано (следующие шаги):** distributed tracing (OpenTelemetry,
§10), `/ready` с проверкой Qdrant, CLI `replay-dlq`, CDC-контракт против
